...
if [ -z "$(ls -A "$PGDATA")" ]; then
    ...
    until pg_isready -h postgres-primary -p 5432 -U ${REP_USER:-replica}; do
    ...
    pg_basebackup -h postgres-primary -D "$PGDATA" -U ${REP_USER:-replica} -v -R -X stream -S ${REPLICATION_SLOT} -P
